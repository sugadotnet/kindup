...
namespace APPBASE.Models
{
    public class AnnouncementCRUD
    {
        ...
        //Constructor
        public AnnouncementCRUD() { } //End public AnnouncementCRUD()
        ...
        public void Update(AnnouncementdetailVM poViewModel)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    poViewModel.SHORT_DESC = poViewModel.FULL_DESC.Substring(0, 200);
                    Announcement oModel = db.Announcements.AsNoTracking().SingleOrDefault(fld => fld.ID == poViewModel.ID);
                    AnnouncementhiddenVM oModelhidden = new AnnouncementhiddenVM();
        ...
        public void Delete(int? id)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Announcement oModel = db.Announcements.Find(id);
                    db.Announcements.Remove(oModel);
    ...
    } //End public class AnnouncementCRUD
