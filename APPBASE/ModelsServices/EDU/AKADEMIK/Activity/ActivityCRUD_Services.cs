...
namespace APPBASE.Models
{
    public class ActivityCRUD
    {
        ...
        //Constructor
        public ActivityCRUD() { } //End public ActivityCRUD()
        public void Create(ActivitydetailVM poViewModel)
        {
            try
            {
        ...
        public void Update(ActivitydetailVM poViewModel)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    poViewModel.SHORT_DESC = poViewModel.FULL_DESC;
                    Activity oModel = db.Activitys.AsNoTracking().SingleOrDefault(fld => fld.ID == poViewModel.ID);
                    ActivityhiddenVM oModelhidden = new ActivityhiddenVM();
        ...
        public void Delete(int? id)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Activity oModel = db.Activitys.Find(id);
                    db.Activitys.Remove(oModel);
