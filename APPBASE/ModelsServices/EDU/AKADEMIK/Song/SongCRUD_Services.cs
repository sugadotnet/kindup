...
namespace APPBASE.Models
{
    public class SongCRUD
    {
        ...
        //Constructor
        public SongCRUD() { } //End public SongCRUD()
        public void Create(SongdetailVM poViewModel)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Song oModel = new Song();
                    //Map Form Data
        ...
        public void Update(SongdetailVM poViewModel)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Song oModel = db.Songs.AsNoTracking().SingleOrDefault(fld => fld.ID == poViewModel.ID);
                    //Map Form Data
                    oModel.InjectFrom(poViewModel);
                    //Set Field Header
        ...
        public void Delete(int? id)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Song oModel = db.Songs.Find(id);
                    db.Songs.Remove(oModel);
                    db.SaveChanges();
                    this.ID = oModel.ID;
                } //End using
            } //End try
