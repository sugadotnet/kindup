...
namespace APPBASE.Models
{
    public class MoodCRUD
    {
        ...
        //Constructor
        public MoodCRUD() { } //End public MoodCRUD()
        public void Create(MooddetailVM poViewModel)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Mood oModel = new Mood();
                    //Map Form Data
        ...
        public void Update(MooddetailVM poViewModel)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Mood oModel = db.Moods.AsNoTracking().SingleOrDefault(fld => fld.ID == poViewModel.ID);
                    //Map Form Data
                    oModel.InjectFrom(poViewModel);
                    //Set Field Header
        ...
        public void Delete(int? id)
        {
            try
            {
                using (var db = new DBMAINContext())
                {
                    Mood oModel = db.Moods.Find(id);
                    db.Moods.Remove(oModel);
                    db.SaveChanges();
                    this.ID = oModel.ID;
                } //End using
            } //End try
