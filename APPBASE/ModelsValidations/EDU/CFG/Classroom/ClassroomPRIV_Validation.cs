...
namespace APPBASE.Models
{
    public partial class Classroom_Validation
    {
        ...
        private void Validate_CLASSTYPE_ID()
        {
            Boolean bIsvalid = true;
            //[CLASSTYPE_ID] - Required
            if (oViewModel.CLASSTYPE_ID == null)
            {
                bIsvalid = false;
                ValidationMSG_VM oMSG = new ValidationMSG_VM();
                oMSG.VAL_ERRID = "CLASSTYPE_ID1";
                oMSG.VAL_ERRMSG = "Kelas harus diisi";
                aValidationMSG.Add(oMSG);
            } //End if
            ////[ID] - Unique
            //if (oDS.isExists_ID(oViewModel.ID))
            ...
            if (!bIsvalid)
            {
                bIsvalid = false;
                ValidationMSG_VM oMSG = new ValidationMSG_VM();
                oMSG.VAL_ERRID = "CLASSTYPE_ID0";
                oMSG.VAL_ERRMSG = "ERROR";
                aValidationMSG.Add(oMSG);
            } //End if
        } //End private void Validate_CLASSTYPE_ID()
